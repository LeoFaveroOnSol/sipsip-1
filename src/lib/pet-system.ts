...
const CinematicShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      color *= smoothstep(0.8, 0.25, len); // Vignette
      color += random(vUv * uTime) * 0.03; // Film Grain
...
// =========================================================
// 2. HELPER PARA RENDERIZAÇÃO (STATE OF THE ART)
// =========================================================
...
export function setupComposer(renderer, scene, camera) {
  ...
  // Cinematic Effects (Grain, Vignette, Aberration)
...
export const PET_TYPES = { FOFO: 'FOFO', CAOS: 'CAOS', CHAD: 'CHAD', DEGEN: 'DEGEN' };
...
class PetBuilder {
  ...
  createMesh(geo, color, rim, outColor, outThick = 0.025, emissive = 0x000000, type = 0) {
    ...
    const matOut = new THREE.ShaderMaterial({
      ...
      uniforms: { uColor: { value: new THREE.Color(outColor) }, uThickness: { value: outThick } },
      side: THREE.BackSide 
    });
  ...
  buildDEGEN(root) {
    ...
    termGroup.position.set(0, -0.1, 0.42); termGroup.rotation.x = -0.15;
    const term = this.createMesh(new THREE.BoxGeometry(0.25, 0.18, 0.04), 0x111111, 0xffffff, 0x000000, 0.01);
    termGroup.add(term);
    const screen = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.12), new THREE.MeshBasicMaterial({ color: new THREE.Color(0x00ff88).multiplyScalar(3.0) })); 
    screen.position.z = 0.025; termGroup.add(screen);
    group.add(termGroup);

    // Candles
    const cCount = 12;
    const cGeo = new THREE.BoxGeometry(0.08, 0.25, 0.02); const wGeo = new THREE.BoxGeometry(0.01, 0.4, 0.01);
    ...
    const dummy = new THREE.Object3D(); const radius = 0.9;
    for(let i=0; i<cCount; i++) {
      const a = (i/cCount)*Math.PI*2;
      dummy.position.set(Math.cos(a)*radius, (Math.random()-0.5)*0.3, Math.sin(a)*radius);
      dummy.rotation.y = -a; dummy.updateMatrix();
      candles.setMatrixAt(i, dummy.matrix); wicks.setMatrixAt(i, dummy.matrix);
      const isUp = Math.random()>0.4;
      candles.setColorAt(i, new THREE.Color(isUp ? 0x00ff88 : 0xff0044).multiplyScalar(2.0));
    }
    const ring = new THREE.Group(); ring.add(candles, wicks); ring.userData.isCandleRing = true;
    group.add(ring); root.add(group);
  ...
  buildFOFO(root) {
    ...
    buildCAOS(root) {
      ...
      for(let i=0; i<count; i++) { basePos[i*3] = geo.attributes.position.getX(i); basePos[i*3+1] = geo.attributes.position.getY(i); basePos[i*3+2] = geo.attributes.position.getZ(i); }
      geo.userData.basePos = basePos;
      const body = this.createMesh(geo, 0x440066, 0x00ff00, 0x220033, 0.03, 0x050011);
      body.position.y = 0.7; body.userData.isChaosBody = true; root.add(body);
      const shard = this.createMesh(new THREE.ConeGeometry(0.1,0.8,4), 0x6600cc, 0x00ff00, 0x220033);
      shard.position.set(0.5,1.2,0); shard.rotation.z=-0.5; root.add(shard);
      const crack = new THREE.Mesh(new THREE.TorusGeometry(0.65,0.03,4,16), new THREE.MeshBasicMaterial({color:0x00ff00}));
      crack.position.y=0.7; crack.rotation.x=1.57; crack.scale.setScalar(1.05); root.add(crack);
  ...
  buildEyes(root, cfg) {
    ...
    const makeEye = (s) => {
      ...
      if(!cfg.degen) pu.scale.set(0.35,0.35,0.8); else pu.scale.set(0.25, 0.25, 0.8);
      pu.position.z = 0.08; u.add(pu);
      const hi = new THREE.Mesh(new THREE.PlaneGeometry(0.05,0.05), new THREE.MeshBasicMaterial({color:0xffffff}));
      hi.position.set(0.05,0.05,0.12); u.add(hi);
      const lid = new THREE.Mesh(new THREE.SphereGeometry(0.13, 24, 24, 0, Math.PI * 2, 0, Math.PI * 0.5), lidMat);
      lid.rotation.x = -0.5; lid.scale.setScalar(1.1); lid.userData.isEyelid = true; u.add(lid);
      if (cfg.degen) {
          const bag = new THREE.Mesh(new THREE.TorusGeometry(0.12, 0.02, 4, 16, 3.14), new THREE.MeshBasicMaterial({color:0xaa0000, transparent:true, opacity:0.15}));
          bag.position.set(0, -0.02, 0.05); bag.rotation.z = 3.14; u.add(bag);
      }
      u.scale.setScalar(s); return u;
    };
    const l = makeEye(cfg.eyeScale); const r = makeEye(cfg.eyeScale);
    const yOff = cfg.relativeToGroup ? 0.7 : 0;
    l.position.set(-cfg.eyeSep, cfg.eyeY+yOff, cfg.eyeZ); r.position.set(cfg.eyeSep, cfg.eyeY+yOff, cfg.eyeZ);
    if(cfg.asymmetric) { r.scale.setScalar(1.5); l.position.y-=0.1; }
    grp.add(l, r); root.add(grp); return {l, r};
  }
}
...
export class PetSystem {
  constructor(scene, camera) {
    ...
    this.light.position.set(5, 8, 5); this.root.add(this.light);
    this.root.add(new THREE.HemisphereLight(0x445566, 0x111122, 1.5));
    
    this.builder = new PetBuilder(scene);
    this.state = 'idle'; this.time = 0; this.stateTimer = 0;
    this.particles = []; this.eyes = null; this.mouth = null;
    this.mouseTarget = new THREE.Vector2();
    this.springX = new Spring(120, 10); this.springY = new Spring(120, 10);

    // Environment: Grid Floor
    const gridMat = new THREE.ShaderMaterial({
      vertexShader: gridVertex, fragmentShader: gridFragment, transparent: true,
  ...
  setType(type) {
    ...
    for(let i=this.root.children.length-1; i>=0; i--) {
        if(this.root.children[i]!==this.floor && this.root.children[i].type === 'Group') this.root.remove(this.root.children[i]);
    }
    this.animRoot = new THREE.Group(); this.root.add(this.animRoot);
    ...
    else if(type === 'DEGEN') cfg = this.builder.buildDEGEN(this.animRoot);
    ...
    const c = type === 'DEGEN' ? 0x00ff88 : (type === 'CAOS' ? 0x8a2be2 : (type === 'FOFO' ? 0xff66aa : 0xaaaaaa));
    this.floor.material.uniforms.uColor.value.setHex(c);
  }

  setState(s) { this.state = s; this.stateTimer = 0; }
  pet() { this.setState('happy'); this.spawnVFX('spark'); }
  feed() { this.setState('eat'); }
  sleep() { this.setState('sleep'); }
  wake() { this.setState('idle'); }
  
  setMouseTarget(x, y) { this.mouseTarget.set(x, y); }

  update(dt) {
    this.time += dt; this.stateTimer += dt;
    this.floor.material.uniforms.uTime.value = this.time;
    if ((this.state==='eat' && this.stateTimer>4.0) || (this.state==='happy' && this.stateTimer>2.0)) this.setState('idle');

    this.springX.target = this.mouseTarget.x; this.springY.target = this.mouseTarget.y;
    this.springX.update(dt); this.springY.update(dt);

    this.root.traverse(o => {
      if(o.material && o.material.uniforms) {
        if(o.material.uniforms.uTime) o.material.uniforms.uTime.value = this.time;
        if(o.material.uniforms.uLightDir) o.material.uniforms.uLightDir.value.copy(new THREE.Vector3(0.5, 1, 1).normalize());
      }
    });

    if(this.type === 'DEGEN') this.updateDegenAnim(dt);
    if(this.type === 'CAOS') this.updateChaosDeform(dt);
    ...
    if(this.food) {
        if(this.state==='eat') {
          ...
          this.food.rotation.y += dt; this.food.rotation.z = Math.sin(this.time*10) * 0.1;
        ...
        } else { this.food.visible = false; this.food.scale.setScalar(1.0); }
    }
  }

  updateDegenAnim(dt) {
    this.animRoot.traverse(c => { if(c.userData.isCandleRing) c.rotation.y += dt * 0.5; });
    if(this.state === 'idle') this.animRoot.position.y = Math.sin(this.time * 4) * 0.03;
  }
  
  updateChaosDeform(dt) {
      let body = null; this.animRoot.traverse(c => { if(c.userData.isChaosBody) body=c; });
      if(!body) return;
      const pos = body.geometry.attributes.position;
      const base = body.geometry.userData.basePos;
  ...
  updateStateAnim(dt) {
     ...
     if(this.state === 'happy') {
        ...
        if (velocity > 0) { targetScale.y = 1.0 + velocity * 0.2; targetScale.x = targetScale.z = 1.0 - velocity * 0.1; } 
        else { targetScale.y = 1.0 + velocity * 0.1; targetScale.x = targetScale.z = 1.0 - velocity * 0.05; }
     } else if (this.state === 'eat') {
        targetRot.x += 0.4 + Math.sin(this.time * 20) * 0.1; targetPos.z += 0.2;
     } else if (this.state === 'sleep') {
        targetRot.z = -1.57; targetPos.y = -0.2;
        const breath = Math.sin(this.time) * 0.03; targetScale.set(1+breath, 1-breath, 1+breath);
     } else if (this.state === 'idle') {
        const breath = Math.sin(this.time * 3) * 0.02; targetPos.y = breath;
     }

     const lerpSpeed = dt * 8;
     if (this.type !== 'DEGEN' || this.state !== 'idle') this.animRoot.position.lerp(targetPos, lerpSpeed);
  ...
  updateEyeLogic(dt) {
    if(!this.eyes) return;
    let blink = 0;
    if(this.state === 'sleep') { blink = 1; if(Math.random()<0.02) this.spawnVFX('zzz'); } 
    else if(Math.random()<0.01) blink = 1; 

    [this.eyes.l, this.eyes.r].forEach(eye => {
      const lid = eye.children.find(c => c.userData.isEyelid);
      if(lid) {
          const startRot = -0.5; const endRot = Math.PI / 2; 
  ...
  updateMouth(dt) {
      ...
      let rZ = 3.14; let sY = 1; let yOff = 0;
      if(this.state === 'eat') { sY = 0.2+Math.abs(Math.sin(this.time*20)); }
      else if(this.state === 'sleep' || (this.state === 'idle' && (this.type==='CAOS' || this.type==='DEGEN'))) { rZ = 0; yOff = -0.05; }
      else if(this.state === 'happy') { sY = 1.3; }
      this.mouth.rotation.z = rZ; this.mouth.scale.y = THREE.MathUtils.lerp(this.mouth.scale.y, sY, 0.2);
      this.mouth.position.y = this.mouth.userData.baseY + yOff;
  }

  spawnVFX(type, pos) {
      const p = pos ? pos.clone() : this.animRoot.position.clone().add(new THREE.Vector3(0,1,0));
      if(type === 'spark') {
        const m = new THREE.Mesh(new THREE.TetrahedronGeometry(0.1), new THREE.MeshBasicMaterial({color:0xffff00}));
        m.position.copy(p); this.root.add(m); this.particles.push({m, life:1, v:new THREE.Vector3(0, 1.5, 0)});
      } else if(type === 'zzz') {
        const c = document.createElement('canvas'); c.width=64; c.height=64;
        const ctx = c.getContext('2d'); ctx.fillStyle='#fff'; ctx.font='bold 40px Arial'; ctx.fillText('Z', 20, 50);
        const m = new THREE.Mesh(new THREE.PlaneGeometry(0.3,0.3), new THREE.MeshBasicMaterial({map:new THREE.CanvasTexture(c), transparent:true, side:2}));
        m.position.copy(this.animRoot.position).add(new THREE.Vector3(0.5,0.5,0));
        this.root.add(m); this.particles.push({m, life:2, v:new THREE.Vector3(0.2, 0.5, 0), t:'z'});
      } else if (type === 'crumb') {
        const m = new THREE.Mesh(new THREE.BoxGeometry(0.05,0.05,0.05), new THREE.MeshBasicMaterial({color:0x8B4513}));
        m.position.copy(p); m.position.x += (Math.random()-0.5)*0.2;
        this.root.add(m); this.particles.push({m, life:1, v:new THREE.Vector3((Math.random()-0.5)*0.5, -1, (Math.random()-0.5)*0.5), t:'c'});
      }
  }
  ...
  updateVFX(dt) {
    for(let i=this.particles.length-1; i>=0; i--) {
        ...
        if(p.t === 'z') { if(this.camera) p.m.lookAt(this.camera.position); p.m.material.opacity = p.life/2; }
        else if (p.t === 'c') { p.m.rotation.x += dt*5; if(p.m.position.y<0) p.life=0; }
        else { p.m.rotation.y += dt*5; p.m.scale.setScalar(p.life); }
